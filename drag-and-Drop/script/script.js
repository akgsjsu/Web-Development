...
function addRect(ctx, x, y) {
	rectCount++;
	ctx.strokeRect(x-50, y-50, 98, 98);
	ctx.fillText("Rectangle "+rectCount, x-30, y+5);
}
...
function addCircle(ctx, x, y) {
	...
	ctx.fillText("Circle "+circleCount, x-20, y+7);
	ctx.stroke();
}
...
function addTriangle(ctx, x, y) {
    ...
    ctx.lineTo(x-48, y+48);
    ctx.closePath();
	ctx.fillText("Triangle "+triangleCount, x-25, y+35);
	ctx.stroke();
}
